...
prepare_cells_table <- function(sceptre_object, gene_locations) {
  ...
  cells_table$assigned_ko[cells_table$assigned_ko == "non-targeting"] <- "non-cutting" # change phrase to be distinct
  ...
  cells_table$assigned_ko_geneloc <- cells_table %>% 
    ...
    pull(assigned_ko)
  cells_table$control_guide <- substr(str_split_i(cells_table$assigned_grna, "_", 2), start=1, stop=2) == "OR" 
...
call_arm_truncations_by_ko <- function(cells_table, ko, distal_read_quantile, gene_locations) {
  ...
  control_cells <- cells_table %>% filter(control_guide, `ChrArm` != target$ChrArm, assigned_ko_geneloc != ko) ###
...
call_arm_truncations <- function(cells_table, distal_read_quantile, gene_locations) {
  ...
  skip_arm_kos <- unique((cells_table %>% filter(is.na(ChrArm)))$assigned_ko)
...
samples <- (cell_line_data_manifest %>% filter(str_detect(Sample, "downsamp")))$Sample
for (cl in samples) {
  ...
  tryCatch({
    ...
    guide_to_gene$gene[guide_to_gene$gene == "non-targeting"] <- "non-cutting" # change phrase to be distinct
    grna_target_data_frame <- data.frame(grna_id=guide_to_gene$guide, grna_target=guide_to_gene$gene) %>%
      mutate(grna_target = ifelse(startsWith(grna_target, 'OR'), 'non-targeting', grna_target)) # ORs are non-targeting
